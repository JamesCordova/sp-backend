...
from app.models import Classroom, FamilyMember, SchoolUserRole, TeacherClassroom, User
...
async def read_me(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    teacher_classrooms = (
        await db.execute(
            select(TeacherClassroom, Classroom)
            .join(Classroom, Classroom.id == TeacherClassroom.classroom_id)
            .where(TeacherClassroom.teacher_user_id == user.id, TeacherClassroom.status == "ACTIVE")
        )
    ).all()
    ...
    return MeOut(
        ...
        teacher_classrooms=[
            {
                "classroom_id": str(tc.classroom_id),
                "name": classroom.name,
                "grade": classroom.grade,
                "section": classroom.section,
            }
            for tc, classroom in teacher_classrooms
        ],
    )
